    ...
    $.each(that.columns, function(i, column) {
      ...
      if (!column.filterControl) {
        ...
        switch (column.filterControl.toLowerCase()) {
          case 'input':
            html.push(sprintf('<input type="text" class="form-control form-control-sm" style="width: 100%; visibility: %s">', isVisible));
            break;
          case 'select':
            html.push(sprintf('<select class="%s form-control form-control-sm" style="width: 100%; visibility: %s"></select>',
              column.field, isVisible))
            break;
          case 'datepicker':
            html.push(sprintf('<input type="text" class="date-filter-control %s form-control form-control-sm" style="width: 100%; visibility: %s">',
              column.field, isVisible));
            break;
        }
      }
  ...
  BootstrapTable.prototype.onColumnSearch = function(event) {
    ...
    if (text) {
      ...
      for(var i in eventos){
        if ($(eventos[i].currentTarget).parent().parent().parent().data('field') == $(event.currentTarget).parent().parent().parent().data('field')){
            var pos = i;
            break;
        }
      }
      if(pos)
        eventos.splice(i,1,event);
      else
        eventos.push(event);
      ...
      this.onSearch(event);
      var eventos2 = new Array();
      for (var i in eventos){
        if ($(eventos[i].currentTarget).parent().parent().parent().data('field') != $(event.currentTarget).parent().parent().parent().data('field')){
          this.onSearch(eventos[i]);
          eventos2.push(eventos[i]);
        }
      }
      eventos = eventos2;
    }
